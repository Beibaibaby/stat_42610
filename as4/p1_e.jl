...
function simulate_network_dynamics(I, deltaI)
    ...
    for t in 1:length(time)
        ...
        da1dt = (-a1 + r1) / tau_a
        dr2dt = (-r2 + S(I2 - g * a2 - w * r1)) / tau_r  # Use I2 here for the dynamics of r2
        da2dt = (-a2 + r2) / tau_a
...
# Loop over deltaI values from -1 to 1 with steps of 0.2
for deltaI in -0.2:0.01:0.0
    max_values = []
    min_values = []
    ...
    # Plotting
    p = plot(I_range, max_values, label="Max r1(t)", xlabel="I", ylabel="r1(t) Value", title="Max and Min of r1(t) vs. I with deltaI=$(round(deltaI, digits=2))", color=:orange)
    ...
    # Save the plot to a file, rounding deltaI for a clean filename
    savefig(p, "deltaI_$(round(deltaI, digits=2)).png")
end
